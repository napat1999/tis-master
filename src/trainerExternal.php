...
<body>
              ...
              <div class="row">
                ...
                <div class="col-md-12">
                  <table id="TrainerTable" class="table table-striped table-bordered nowrap" style="width:100%">
                    ...
                    <tbody>
                      <?php
                      $sql = "select * from trainer where employeeNo is NULL order by thai_name";
                      $result = json_decode(pgQuery($sql), true);
                      if ($result['code'] == "200") {
                        for ($i = 0; $i < count($result) - 1; $i++) {
                          //$urlImage = SignUrlObjectOnBucket("", $result[$i]['imagepath'], "1");
                          echo "<tr>\n";
                          ...
                          echo "data-alt='" . $result[$i]['thai_name'] . "' src='".$result[$i]['imagepath']."'>";
                          echo "</div>";
                          echo "</td>";
                          echo "<td>";
                          echo "<a href='trainerExternalView.php?trainerID=" . $result[$i]['trainerID'] . "'>" . $result[$i]['th_initial'] . " " . $result[$i]['thai_name'] . "</a>";
                          //if ($_SESSION["isadmin"] == "1" or $_SESSION["istraininghq"] == "1" or $_SESSION["istrainingro"] == "1") {
                            echo "<br/>";
                            echo "<a href='trainerExternalEdit.php?trainerID=" . $result[$i]['trainerID'] . "'><i class='fas fa-user-edit'></i> แก้ไข</a>";
                            echo "<br/>";
                            echo "<a style='color:red;' href='javascript:trainerDelete(\"" . $result[$i]['trainerID'] . "\",\"" . $result[$i]['th_initial'] . " " . $result[$i]['thai_name'] . "\")'><i class='fas fa-user-minus'></i> ลบ</a>";
                            echo "</td>";
                          //}
                          ...
                          echo "<td>" . $result[$i]['email'] . "<br>โทร : " . $result[$i]['telephone'] . "<br><p style='color:blue'>ติดต่อ " . $result[$i]['contact_p'] . "<br>" . $result[$i]['contact_tel'] . "<br>" . $result[$i]['contact_email'] . "</p></td>";

                          echo "</tr>\n";
                        }
                        echo "<br/>";
                        echo "<a href='trainerExternalAdd.php' >เพิ่มวิทยากรภายนอก</a>";
                        echo "<br/>";
                      } else {
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      if ($_SESSION["isadmin"] == "1" or $_SESSION["istraininghq"] == "1" or $_SESSION["istrainingro"] == "1") {
      ?>
        $("div.toolbar").html('<a class="btn btn-app" href="trainerExternalEdit.php"><i class="fas fa-user-plus"></i> เพิ่มวิทยากร</a>');
      <?php
      }
      ?>
    });
    ...
    function trainerDelete(id, textContant) {
      bootbox.confirm({
        closeButton: false,
        title: "กรุณายืนยัน การลบข้อมูลวิทยากรภายนอก ?",
        message: textContant,
        size: 'small',
        animate: true,
        centerVertical: true,
        ...
        callback: function(result) {
          if (result) {
            $.ajax({
              type: "POST",
              url: "db/deleteTrainerExternal.php",
              data: "trainerID=" + id,
              beforeSend: function() {
                $('#loading').show();
              },
